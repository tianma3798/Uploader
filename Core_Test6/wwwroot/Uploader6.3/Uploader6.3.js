...
(function ($) {
    ...
    var uploader = function (elem, opts) {
        ...
        //根据当前链接处理uploadType  如果以 ws://或者 wss://开头则是使用webscoket方式
        if (this.opts.url.indexOf('ws://') != -1 ||
            this.opts.url.indexOf('wss://') != -1)
    ...
    uploader.prototype = {
        ...
        initBtn: function () {
            ...
            //绑定事件
            if (_opts.type == UPLOADTYPE.single) {
                _this.bindBtn();
            } else {
                _this.initImgDouble();
            }
            ...
            file.change(function () {
                ...
                if (_opts.more) {
                    ...
                    var thisInter = setInterval(function () {
                        ...
                        //判断终止
                        if (i == fileList.length - 1 || fileList.length == 0) {
                            clearInterval(thisInter);
                            if (_opts.onAllSuccess)
                                _opts.onAllSuccess();
                        }
                    }, 200);
                } else {
